...
import { upsertRegistrationReception } from "@/lib/registrations";
...
export async function POST(req: Request, ctx: RouteContext<"/api/admin/registrations/[id]/reception">) {
  ...
  try {
    await upsertRegistrationReception(id, parsed.data);
  } catch {
    return NextResponse.json({ ok: false, error: "保存失败" }, { status: 400 });
  }
